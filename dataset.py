...
# ==========================================
# 本地测试代码 (请在本地创建两张假图片和假标签进行测试)
# ==========================================
if __name__ == '__main__':
    print("🚀 启动工业级 Dataloader 数据管道测试...")

    # 模拟创建临时文件夹和假数据 (仅供验证代码逻辑)
    os.makedirs("dummy_images", exist_ok=True)
    os.makedirs("dummy_labels", exist_ok=True)

    # 制造一张 1920x1080 的纯黑图片模拟工业相机抓拍
    dummy_img = np.zeros((1080, 1920, 3), dtype=np.uint8)
    cv2.imwrite("dummy_images/test_01.jpg", dummy_img)

    # 制造一个 YOLO 格式的 txt 标签 (类别 1, 归一化中心点 0.5 0.5, 宽 0.1, 高 0.1)
    with open("dummy_labels/test_01.txt", "w") as f:
    ...
    try:
        ...
        # 抽取第 0 个样本
        img, labels, bboxes = dataset[0]

        print("✅ 数据管道读取成功！")
        print(f"输出图像张量维度: {img.shape}  -> 期待: torch.Size([3, 640, 640])")
        print(f"输出标签张量维度: {labels.shape}  -> 期待: torch.Size([1, 1])")
        print(f"输出坐标张量维度: {bboxes.shape}  -> 期待: torch.Size([1, 4])")

        print(f"\n物理坐标映射验证:")
        print(f"归一化坐标 [0.5, 0.5, 0.1, 0.1] 在 1920x1080 原图经过 Letterbox 后，")
        print(f"被精准映射到了 640x640 画布上的绝对坐标 (x1, y1, x2, y2): \n{bboxes[0]}")
